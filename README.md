# 💻 English for Information Technology

Materi pembelajaran mata kuliah **English for Information Technology** untuk Program Studi Informatika, Universitas Pertahanan RI.

---

## 📋 Informasi Mata Kuliah

| Komponen | Keterangan |
|----------|------------|
| **Kode** | EIT-201 |
| **SKS** | 2 SKS (1 SKS Teori + 1 SKS Praktek) |
| **Semester** | 2 |
| **Prasyarat** | — |
| **Level Bahasa** | CEFR B2 (Upper Intermediate) |
| **Pengampu** | Anindito, S.Kom., S.S., S.H., M.TI., CHFI. |

---

## 📄 Silabus

[📥 Lihat Silabus Lengkap](silabus.md)

### Deskripsi Singkat

Mata kuliah ini dirancang untuk mengembangkan kemampuan bahasa Inggris mahasiswa dalam konteks Teknologi Informasi pada level upper intermediate (CEFR B2). Kurikulum terintegrasi mencakup empat keterampilan utama: Listening, Speaking, Reading, dan Writing, dengan penekanan pada kosa kata teknis dan komunikasi profesional di bidang IT. Mahasiswa akan mampu memahami dan memproduksi wacana teknis, berpartisipasi dalam diskusi profesional, membaca dokumentasi teknis, dan menulis berbagai jenis teks formal di lingkungan IT.

### Capaian Pembelajaran

Setelah menyelesaikan mata kuliah ini, mahasiswa mampu:

1. Memahami dan menggunakan kosa kata teknis IT tingkat menengah-lanjut dalam konteks lisan dan tertulis **(CO1)**
2. Mengkomunikasikan informasi teknis secara efektif dalam lingkungan IT profesional **(CO2)**
3. Membaca dan memahami berbagai jenis teks teknis, termasuk dokumentasi sistem dan artikel industri **(CO3)**
4. Menulis berbagai jenis teks profesional di bidang IT, termasuk laporan teknis dan proposal **(CO4)**
5. Memahami wacana lisan dalam konteks IT profesional **(CO5)**
6. Menerapkan struktur gramatikal yang tepat dan kompleks untuk berkomunikasi secara efektif **(CO6)**

---

## 🎯 Daftar Materi Pembelajaran

### Bagian 1: Fondasi IT English (Pertemuan 1–7)

| # | Topik | Grammar Focus | Self Study |
|:-:|-------|---------------|:----------:|
| 01 | Introduction to IT English & Computer Systems | Present Simple & Continuous | [📚](p01/self-study.md) |
| 02 | Computer Hardware & Specifications | Countable/Uncountable; Comparatives | [📚](p02/self-study.md) |
| 03 | Operating Systems & Software | Present Perfect; Imperatives & Modals | [📚](p03/self-study.md) |
| 04 | Software Development & Programming | Past Simple & Continuous; Passive Voice | [📚](p04/self-study.md) |
| 05 | Computer Networking | Conditionals Type 0 & 1; Phrasal Verbs | [📚](p05/self-study.md) |
| 06 | Cybersecurity | Modals (must/should); Future Forms | [📚](p06/self-study.md) |
| 07 | Cloud Computing | Comparatives (review); Articles | [📚](p07/self-study.md) |
| 08 | **Ujian Tengah Semester (UTS)** | — | — |

---

### Bagian 2: IT English Lanjutan (Pertemuan 9–15)

| # | Topik | Grammar Focus | Self Study |
|:-:|-------|---------------|:----------:|
| 09 | Web Development & Technologies | Relative Clauses; Gerunds & Infinitives | [📚](p09/self-study.md) |
| 10 | Databases & Data Management | Reported Speech; Prepositions | [📚](p10/self-study.md) |
| 11 | Artificial Intelligence & Machine Learning | Future Forms; Adjectives & Adverbs | [📚](p11/self-study.md) |
| 12 | Data Science & Analytics | Passive Voice (advanced); Conditionals 2 & 3 | [📚](p12/self-study.md) |
| 13 | IT Project Management | Noun Compounds; Relative Clauses (review) | [📚](p13/self-study.md) |
| 14 | Emerging Technologies & Innovation | Modals (possibility); Time Expressions | [📚](p14/self-study.md) |
| 15 | IT Ethics, Security & Future Trends | Future Perfect & Continuous; Speculation Modals | [📚](p15/self-study.md) |
| 16 | **Ujian Akhir Semester (UAS)** | — | — |

---

## 📖 Referensi

### Referensi Utama

1. Murphy, R. (2019). *English Grammar in Use* (5th Ed.). Cambridge University Press.
2. Olejniczak, M. (2011). *English for Information Technology: Course Book, Volume 1*. Pearson Education.

### Referensi Pendukung

3. Materi Listening AI-Generated (Script + Google Cloud TTS Neural2)
4. Supplementary technical articles & documentation

### Sumber Online

- [Murphy's Grammar in Use Online](https://www.cambridge.org/elt/grammarinuse)
- [IT English Vocabulary — Olejniczak Companion Site](https://www.pearsonelt.com)
- [TED Talks Technology](https://www.ted.com/topics/technology)
- [BBC Learning English — Work & Career](https://www.bbc.co.uk/learningenglish)

---

## 🛠️ Cara Menggunakan Materi Self-Study

### 📚 Belajar Mandiri (Self Study)

Klik link **📚** untuk mengakses panduan belajar mandiri yang mencakup:

- **Vocabulary flashcards** — daftar kosa kata siap untuk Quizlet atau Anki
- **Grammar exercises** — latihan mandiri berdasarkan referensi Murphy's *English Grammar in Use*
- **Extension reading** — artikel teknis terkurasi dari sumber terpercaya
- **Extension listening & video** — playlist YouTube dan podcast IT berbahasa Inggris
- **Writing task** — tugas menulis mingguan beserta rubrik penilaian
- **Answer key** — kunci jawaban untuk semua latihan mandiri

---

## 📂 Struktur Repository

```
english-it/
├── README.md                   # Halaman ini
├── LICENSE                     # Lisensi CC BY 4.0
├── silabus.md                  # Silabus lengkap
│
├── p01/                        # Pertemuan 01: Introduction to IT English
│   └── self-study.md
│
├── p02/                        # Pertemuan 02: Computer Hardware
│   └── self-study.md
│
├── p03/                        # Pertemuan 03: Operating Systems
│   └── self-study.md
│
├── p04/                        # Pertemuan 04: Software Development
│   └── self-study.md
│
├── p05/                        # Pertemuan 05: Computer Networking
│   └── self-study.md
│
├── p06/                        # Pertemuan 06: Cybersecurity
│   └── self-study.md
│
├── p07/                        # Pertemuan 07: Cloud Computing
│   └── self-study.md
│
├── p09/                        # Pertemuan 09: Web Development
│   └── self-study.md
│
├── p10/                        # Pertemuan 10: Databases
│   └── self-study.md
│
├── p11/                        # Pertemuan 11: AI & Machine Learning
│   └── self-study.md
│
├── p12/                        # Pertemuan 12: Data Science
│   └── self-study.md
│
├── p13/                        # Pertemuan 13: IT Project Management
│   └── self-study.md
│
├── p14/                        # Pertemuan 14: Emerging Technologies
│   └── self-study.md
│
└── p15/                        # Pertemuan 15: IT Ethics & Future Trends
    └── self-study.md
```

---

## 🔗 Tautan Penting

| Tautan | Keterangan |
|--------|------------|
| [🏠 Portal Utama](https://informatikaunhan.github.io/portal/) | Daftar semua mata kuliah |
| [📚 Self-Study Pertemuan 1](https://github.com/informatikaunhan/english-it/blob/main/p01/self-study.md) | Mulai dari sini |

---

## 📊 Sistem Penilaian

| Komponen | Bobot |
|----------|-------|
| Kehadiran dan Partisipasi Aktif | 10% |
| Tugas Mingguan (Weekly Assignments) | 20% |
| Ujian Tengah Semester (UTS) — Pertemuan 8 | 30% |
| Ujian Akhir Semester (UAS) — Pertemuan 16 | 40% |

### Format Ujian (UTS dan UAS)

| Komponen | Bobot |
|----------|-------|
| Listening Comprehension | 20% |
| Reading Comprehension | 25% |
| Grammar & Vocabulary | 20% |
| Writing Task | 25% |
| Speaking Assessment (dilakukan terpisah) | 10% |

---

## License / Lisensi

This repository is licensed under the **Creative Commons Attribution 4.0 International (CC BY 4.0)**.

You are free to:
- **Share** — copy and redistribute the material in any medium or format
- **Adapt** — remix, transform, and build upon the material for any purpose, even commercially

Under the following terms:
- **Attribution** — You must give appropriate credit to the author

---

Repositori ini dilisensikan di bawah **Creative Commons Attribution 4.0 International (CC BY 4.0)**.

Anda bebas untuk:
- **Berbagi** — menyalin dan menyebarluaskan materi dalam bentuk atau format apapun
- **Adaptasi** — mengubah, menggubah, dan membuat turunan dari materi untuk tujuan apapun, termasuk komersial

Dengan ketentuan:
- **Atribusi** — Anda harus mencantumkan kredit kepada penulis

[![CC BY 4.0](https://licensebuttons.net/l/by/4.0/88x31.png)](https://creativecommons.org/licenses/by/4.0/)

© 2026 Anindito
